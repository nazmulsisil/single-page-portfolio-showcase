...
const patATap = () => {
  const currentCanvas = document.getElementById('patATap__canvas');
  ...
  tool.onKeyDown = e => {
    if (data[e.key]) {
      ...
      const randSize = 50 + 450 * Math.random();
      const randCircle = new paper.Path.Circle(point, randSize);
      randCircle.fillColor = data[e.key].color;
      data[e.key].sound.play();
      circlesArr.push(randCircle);
    }
  };

  paper.view.onFrame = () => {
    circlesArr.forEach((circle, i) => {
